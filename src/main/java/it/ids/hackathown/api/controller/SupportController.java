...
import it.ids.hackathown.service.SupportService;
...
public class SupportController {
    ...
    public ResponseEntity<SupportRequestResponse> createSupportRequest(
        ...
        SupportRequestResponse response = mapper.toResponse(
            supportService.createSupportRequest(hackathonId, currentUserId, request.message())
        );
    ...
    public List<SupportRequestResponse> listSupportRequests(
        ...
        return supportService.listSupportRequests(hackathonId, currentUserId).stream().map(mapper::toResponse).toList();
    }

    @PostMapping("/support-requests/{supportRequestId}/propose-call")
    public ResponseEntity<CallProposalResponse> proposeCall(
        ...
        CallProposalResponse response = mapper.toResponse(
            supportService.proposeCall(supportRequestId, currentUserId, request.proposedSlots())
        );
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }
}
